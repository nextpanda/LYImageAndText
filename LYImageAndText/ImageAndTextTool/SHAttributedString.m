...
@implementation SHAttributedString
...
{
    //======================如果string为空或者是空字符串，直接返回nil
    if (string==nil || [string isEqualToString:@""]) return nil;
    
    NSArray *rangeArr = [SHMatching urlRangeArrWithString:string];
    ...
    if (rangeArr)
    {
        ...
        }else
        {
            ...
            NSArray *url_Arr = [SHMatching urlArrWithString:string];
...
+(NSAttributedString*)attributedStringWithAttributedString:(NSAttributedString*)attributedString insertImage:(UIImage*)image atIndex:(NSInteger)index
{
    ...
    textAttachment.bounds = CGRectMake(0, -attstr.size.height/5, attstr.size.height, attstr.size.height);
...
+(NSAttributedString*)attributedPhoneStringWithString:(NSAttributedString*)attString insertImage:(UIImage *)image orPhoneNameString:(NSString*)phoneName isCall:(BOOL)isCall
{
    ...
    NSArray *rangeArr = [SHMatching phoneNumRangeArrWithString:originStr];
    NSArray *phoneNum_Arr = [SHMatching phoneNumArrWithString:originStr];
...
//    NSMutableAttributedString* attStr = [[NSMutableAttributedString alloc]initWithAttributedString:attString];
    ...
    NSAttributedString* attStr = [SHAttributedString attributedStringWithString:attString insertImage:image orStringName:phoneName linkUrlString:callStr rangeArr:rangeArr originStringArr:phoneNum_Arr];
...
+(NSAttributedString*)attributedEmailStringWithString:(NSAttributedString*)attString insertImage:(UIImage *)image orEmailNameString:(NSString*)emailName
{
    ...
    NSArray *rangeArr = [SHMatching emailRangeArrWithString:originStr];
    NSArray *emailNum_Arr = [SHMatching emailStringArrWithString:originStr];

    NSAttributedString* attStr = [SHAttributedString attributedStringWithString:attString insertImage:image orStringName:emailName linkUrlString:@"mailto://" rangeArr:rangeArr originStringArr:emailNum_Arr];
